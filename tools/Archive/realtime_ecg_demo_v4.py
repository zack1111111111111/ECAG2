...
class ECGRealtimeDemo:
    def __init__(self, scenarios_dir: Path):
        ...
        # ---- Agent team state ----
        # Agent runs on a background thread so the UI stays responsive.
        self.agent_thread       = None
        self.agent_running      = False
        self.last_agent_result  = None      # filled by background thread
    ...
    def update(self, frame):
        ...
        # AI feature summary: every classify tick (preliminary, NOT a diagnosis)
        if (self.frame_count % self.frames_per_classify == 0
            ...
            self.text_diag.set_text(
                ...
                f"{expl}.   "
                f"(Final diagnosis to be made by agent team)"
            )
